-- =====================================================
-- SCRIPT DE EXPANSÃO DO CADASTRO DE FUNCIONÁRIOS - VERSÃO CORRIGIDA
...
-- Políticas RLS básicas para employee_documents
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can view employee_documents from their company'
  ) THEN
    CREATE POLICY "Users can view employee_documents from their company" ON rh.employee_documents
      FOR SELECT USING (
        company_id IN (
          SELECT company_id FROM core.users WHERE id = auth.uid()
        )
      );
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'rh' 
    AND tablename = 'employee_documents' 
    AND policyname = 'Users can insert employee_documents for their company'
  ) THEN
    CREATE POLICY "Users can insert employee_documents for their company" ON rh.employee_documents
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'rh' 
    AND tablename = 'employee_documents' 
    AND policyname = 'Users can update employee_documents from their company'
  ) THEN
    CREATE POLICY "Users can update employee_documents from their company" ON rh.employee_documents
      FOR UPDATE USING (
        company_id IN (
          SELECT company_id FROM core.users WHERE id = auth.uid()
        )
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'rh' 
    AND tablename = 'employee_documents' 
    AND policyname = 'Users can delete employee_documents from their company'
  ) THEN
    CREATE POLICY "Users can delete employee_documents from their company" ON rh.employee_documents
      FOR DELETE USING (
        company_id IN (
          SELECT company_id FROM core.users WHERE id = auth.uid()
        )
...
-- Políticas RLS básicas para employee_addresses
CREATE POLICY IF NOT EXISTS "Users can view employee_addresses from their company" ON rh.employee_addresses
  FOR SELECT USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can insert employee_addresses for their company" ON rh.employee_addresses
  FOR INSERT WITH CHECK (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can update employee_addresses from their company" ON rh.employee_addresses
  FOR UPDATE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can delete employee_addresses from their company" ON rh.employee_addresses
  FOR DELETE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
-- Políticas RLS básicas para employee_spouses
CREATE POLICY IF NOT EXISTS "Users can view employee_spouses from their company" ON rh.employee_spouses
  FOR SELECT USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can insert employee_spouses for their company" ON rh.employee_spouses
  FOR INSERT WITH CHECK (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can update employee_spouses from their company" ON rh.employee_spouses
  FOR UPDATE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can delete employee_spouses from their company" ON rh.employee_spouses
  FOR DELETE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
-- Políticas RLS básicas para employee_bank_accounts
CREATE POLICY IF NOT EXISTS "Users can view employee_bank_accounts from their company" ON rh.employee_bank_accounts
  FOR SELECT USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can insert employee_bank_accounts for their company" ON rh.employee_bank_accounts
  FOR INSERT WITH CHECK (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can update employee_bank_accounts from their company" ON rh.employee_bank_accounts
  FOR UPDATE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can delete employee_bank_accounts from their company" ON rh.employee_bank_accounts
  FOR DELETE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
-- Políticas RLS básicas para employee_education
CREATE POLICY IF NOT EXISTS "Users can view employee_education from their company" ON rh.employee_education
  FOR SELECT USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can insert employee_education for their company" ON rh.employee_education
  FOR INSERT WITH CHECK (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can update employee_education from their company" ON rh.employee_education
  FOR UPDATE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can delete employee_education from their company" ON rh.employee_education
  FOR DELETE USING (
    company_id IN (
      SELECT company_id FROM core.users WHERE id = auth.uid()
    )
...
-- 12. CRIAR CONSTRAINTS DE VALIDAÇÃO
-- =====================================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE rh.employee_documents 
    ADD CONSTRAINT check_cpf_format 
    CHECK (cpf IS NULL OR cpf ~ '^\d{11}$');
  END IF;

  -- Constraint para validar CEP
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    ...
    ALTER TABLE rh.employee_addresses 
...
-- 10. ✅ Funções RPC para operações comuns
