-- =====================================================
-- SCRIPT SEGURO DE EXPANSÃO DO CADASTRO DE FUNCIONÁRIOS
...
-- 6. EXPANDIR TABELA EMPLOYEES COM CAMPOS ADICIONAIS (VERSÃO SEGURA)
-- =====================================================
DO $$
BEGIN
  -- Verificar e adicionar colunas uma por uma
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'position_id') THEN
    ALTER TABLE rh.employees ADD COLUMN position_id uuid NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'work_schedule_id') THEN
    ALTER TABLE rh.employees ADD COLUMN work_schedule_id uuid NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'department_id') THEN
    ALTER TABLE rh.employees ADD COLUMN department_id uuid NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'manager_id') THEN
    ALTER TABLE rh.employees ADD COLUMN manager_id uuid NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'salario_base') THEN
    ALTER TABLE rh.employees ADD COLUMN salario_base numeric(10,2) NULL;
  END IF;
  
  -- Verificar se nivel_escolaridade já existe antes de adicionar
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'nivel_escolaridade') THEN
    ALTER TABLE rh.employees ADD COLUMN nivel_escolaridade text NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'telefone') THEN
    ALTER TABLE rh.employees ADD COLUMN telefone text NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'email') THEN
    ALTER TABLE rh.employees ADD COLUMN email text NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'estado_civil') THEN
    ALTER TABLE rh.employees ADD COLUMN estado_civil text NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'nacionalidade') THEN
    ALTER TABLE rh.employees ADD COLUMN nacionalidade text NULL DEFAULT 'Brasileira';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'naturalidade') THEN
    ALTER TABLE rh.employees ADD COLUMN naturalidade text NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'nome_mae') THEN
    ALTER TABLE rh.employees ADD COLUMN nome_mae text NULL;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employees' AND table_schema = 'rh' AND column_name = 'nome_pai') THEN
    ALTER TABLE rh.employees ADD COLUMN nome_pai text NULL;
  END IF;
END $$;

-- 7. ADICIONAR FOREIGN KEYS PARA OS NOVOS CAMPOS
-- =====================================================
DO $$
BEGIN
...
-- 8. HABILITAR RLS (ROW LEVEL SECURITY) PARA AS NOVAS TABELAS
-- =====================================================
...
-- 9. CRIAR TRIGGERS DE AUDITORIA PARA AS NOVAS TABELAS
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE trigger_name = 'audit_employee_documents' 
    AND event_object_table = 'employee_documents' 
    AND event_object_schema = 'rh'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'audit_employee_addresses' 
    AND event_object_table = 'employee_addresses' 
    AND event_object_schema = 'rh'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'audit_employee_spouses' 
    AND event_object_table = 'employee_spouses' 
    AND event_object_schema = 'rh'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'audit_employee_bank_accounts' 
    AND event_object_table = 'employee_bank_accounts' 
    AND event_object_schema = 'rh'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'audit_employee_education' 
    AND event_object_table = 'employee_education' 
    AND event_object_schema = 'rh'
  ) THEN
...
-- 10. CRIAR VIEWS PARA FACILITAR CONSULTAS
-- =====================================================
...
-- 11. CRIAR FUNÇÕES RPC PARA OPERAÇÕES COMUNS
-- =====================================================
...
-- 13. COMENTÁRIOS DAS TABELAS E CAMPOS
-- =====================================================
