...
import { ArrowLeft, Gift, Users, DollarSign, Filter, Download, Plus, TrendingUp } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { BenefitsManagement } from '@/components/rh';

export default function BenefitsManagementPage() {
  const { user } = useAuth();
  const navigate = useNavigate();
  
  // Buscar a empresa do usuário logado usando o hook correto
  const { data: userCompany, isLoading: loadingCompany } = useCompany();
      ...
      <div className="container mx-auto py-6 px-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-2">
                  Gerencie benefícios e vantagens dos funcionários da empresa <strong>{userCompany.nome_fantasia || userCompany.razao_social}</strong>
                </p>
              </div>
            </div>
        ...
        <div className="mb-6">
          <nav className="flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            <span>/</span>
            <span className="text-foreground font-medium">Benefícios</span>
          </nav>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Com benefícios ativos</p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Benefícios por Funcionário</CardTitle>
        ...
        {/* Conteúdo principal */}
        <BenefitsManagement companyId={userCompany.id} />
      </div>
    </div>
  );
}
